...
/**
 * /sheng-yen-specific follow-up renderer: 4-card grid below the
 * media citations. Lives separately from the shared
 * FollowupSuggestions component so we can iterate on layout for the
 * A/V tab without touching main chat or /events. Uses the same
 * `useFollowupSuggestions` hook so the data source stays identical.
 */
export const ShengYenFollowups: FC = () => {
  const aui = useAui();
