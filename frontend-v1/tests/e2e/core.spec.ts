...
import {
  ...
  signInWithDevToken,
  trackRequests,
  waitForRunDebugEvent,
  waitForAssistantTextStart,
  waitForRunToComplete,
  waitForVisibleAssistantText,
} from "./helpers";
...
test("core: anonymous long query shows visible assistant text", async ({ page }) => {
  test.setTimeout(60_000);
  await expect(page.getByText("Anonymous")).toBeVisible();
  ...
  await sendMessage(page, longPrompt);
  await completionResponsePromise;
  await waitForVisibleAssistantText(page, 20_000);
...
test("core: signed-in long query starts streaming", async ({ page }) => {
  test.setTimeout(60_000);
  ...
  await threadCreateResponsePromise;
  await threadRunResponsePromise;
  await waitForRunDebugEvent(page, "first_assistant_text_committed", 20_000);
